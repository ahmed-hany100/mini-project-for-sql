...
-- Mini-Project 2: Query Optimization
-- Role: Analytical Reports / Optimization (Member 4)
-- Built on the schema created by Member 1 (Schema_2.sql)
...
-- ============================================================
-- Run this section FIRST, before creating any index below.
-- Turn these two settings on so SQL Server prints how many pages
-- it had to read and how long the query took. Copy the messages
-- from the "Messages" tab into the report as the "before" numbers.
--
-- In SSMS also press Ctrl + M (Include Actual Execution Plan)
-- before running, and screenshot the plan. Without indexes the
-- plan on fact_sales will show a Clustered Index Scan, which
-- means the whole table is being read every time.
-- ============================================================
...
-- ============================================================
-- fact_sales already has a clustered index on sales_row_id
-- (the primary key), but none of our queries filter or join on
-- sales_row_id. Every query joins on a foreign key instead, so
-- each one had to scan the full table.
--
-- The indexes below are non-clustered indexes on those foreign
-- key columns. The INCLUDE part stores the measure columns
-- (sales, profit, quantity) inside the index itself, so the
-- query can be answered from the index alone without going back
-- to the table. This is called a covering index.
--
-- The IF EXISTS checks let the script be run more than once
...
-- ============================================================
-- Run the exact same three test queries again and compare.
-- The logical reads and the elapsed time should both drop, and
-- the execution plan should now show an Index Seek or an Index
-- Scan on the new non-clustered index instead of a full
-- Clustered Index Scan on fact_sales.
-- ============================================================
...
-- ============================================================
-- Run this after executing the project queries a few times.
-- user_seeks and user_scans show how many times each index was
-- used. An index that stays at zero is not helping and would be
-- worth dropping, because every index also slows down inserts.
-- ============================================================
